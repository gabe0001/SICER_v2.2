...
def get_window_counts(iterator, genome_data, window_size, fragment_size, scaling_factor):
    ...
    for read in iterator:
        if read.iv.chrom in genome_data:
            if read.iv.start < 0:
                continue
            if read.iv.start > genome_data[read.iv.chrom]:
                continue
            ...
            read_counts[read.iv.chrom][window_start] +=1
...
def get_window_counts_pe(iterator, genome_data, window_size, scaling_factor):
    # dictionary to store read counts
    read_counts = {}
    # dictionary to store read count in each window
    window_counts_dict = {}
    ...
    for chrom in genome_data:
        # dict of dicts read_counts[chrom][window_start], float for pile-up implementation
    ...
    for chrom in genome_data:
        for window_start in read_counts[chrom]:
            ...
            normalized_count = float(read_count) * float(scaling_factor) / float(proper_pairs)
            window_end = window_start + window_size 
            window = HTSeq.GenomicInterval(chrom, window_start, window_end)
            try:
                normalized_window_array[window] = normalized_count
...
def find_islands(window_counts_dict, total_reads, gap, window_size, genome_data,
    ...
    for i in islands_list:
        ...
        try:
            ...
            #print "i['chip'] = " + str(i['chip'])
            print "i['island'] = " + str(i['island'])
...
def paired_to_single_read(bamfile):
    ...
    #os.system('bamToBed -i %s > %s' % (bamfile[:-4]+".bam", bamfile[:-4]+"_sorted.bed"))
    ...
    for mate1, mate2 in pe_iterator:
        try:
            ...
            #    continue
                ...
                write_to_outfile(outfile, frag_iv, mate1)
...
def remove_redundant_reads_bam(file_name, out_file_name, cutoff, genome_data):
    ...
    for read in bam_reader:
        ...
        # make sure read is located on genome
        if start < 0:
            #print "Illegitimate read with start less than zero is ignored: " #+ read.read.name
            continue
        end = read.iv.end
        if end > genome_data[chr]:
            #print "Illegitimate read with end beyond chromosome length " + str(genome_data[chr]) + " is ignored: " \
...
def count_reads_in_islands_ref(islands_list, window_size, read_counts, control_read_counts):
    ...
    for island in islands_list:
        ...
        # calculate chip read count in island
        chip_count = get_read_count_in_interval(island['island'], window_size, read_counts)
        
        # calculate control read count in island
        control_count = get_read_count_in_interval(island['island'], window_size, control_read_counts)
...
def get_read_count_in_interval(region, window_size, read_counts):

    current_window = region.start
    read_count = 0
    
    while current_window <= region.end:
        if current_window in read_counts[region.chrom]:
            read_count += read_counts[region.chrom][current_window]
...
def get_pvalue_fc_write_islandsummary(islands_list, chip_library_size, control_library_size, genome_fraction,
    ...
    for island in islands_list:
        ...
        try:
            ...
            #print "IndexError in assignment"
            #print "island_array[island['island']] = " + str(island['island'])
            pass
